...
def train_one_epoch(model, optimizer, data_loader, device, epoch,
    ...
    scaler = amp.GradScaler(enabled=enable_amp)#自动混合精度
    ...
    #下面为枚举函数
    #enumerate使这个元组列表生成索引+元组对，每个元组中共有五个元素
    for i, (imgs, targets, paths, _, _) in enumerate(metric_logger.log_every(data_loader, print_freq, header)):
        # ni 统计从epoch0开始的所有batch数，即好几轮加起来的batch数
        ni = i + nb * epoch  # number integrated batches (since train start),这里比较巧妙epoch为0的时候，ni=i就是当前的batch数
        imgs = imgs.to(device).float() / 255.0  # uint8 to float32, 0 - 255 to 0.0 - 1.0，uni8类型是8位无符号整数
