...
import json #JSON(JavaScript Object Notation)  用python编码和解码json对象
import shutil #拷贝文件
...
# 拼接出voc的images目录，xml目录，
voc_pathh=os.path.join(voc_root, voc_version)
path= os.path.exists(voc_root)
voc_images_path = os.path.join(voc_root, voc_version, "JPEGImages") #拼接出images的目录
voc_xml_path = os.path.join(voc_root, voc_version, "Annotations") #拼接出xml的目录
train_txt_path = os.path.join(voc_root, voc_version, "ImageSets", "Main", train_txt) #拼接训练集的txt目录
val_txt_path = os.path.join(voc_root, voc_version, "ImageSets", "Main", val_txt) #拼接测试集的txt目录

# 检查文件/文件夹都是否存在
path = os.path.exists(train_txt_path)#测试语句 #当前目录
...
def parse_xml_to_dict(xml):
    """
    式，参考tensorflow的recursive_parse_xml_将xml文件解析成字典形to_dict
    Args：
