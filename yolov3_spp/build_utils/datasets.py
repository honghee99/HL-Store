...
class LoadImagesAndLabels(Dataset):  # for training/testing
    def __init__(self,
                 ...
                 rect=False,  # 是否使用rectangular training 矩形训练
                 cache_images=False,  # 是否缓存图片到内存中
                 single_cls=False, pad=0.0, rank=-1):

        try:
            path = str(Path(path))#处理转义字符\
            # parent = str(Path(path).parent) + os.sep
            if os.path.isfile(path):  # file
                # 读取对应my_train/val_data.txt文件，读取每一行的图片路径信息
                with open(path, "r") as f:
                    f = f.read().splitlines()#如果是文件，一行一行读
            else:
                raise Exception("%s does not exist" % path)

            # （下面这句话很重要，把txt中图片路径的信息转到列表中）检查每张图片后缀格式是否在支持的列表中，保存支持的图像路径，用高级循环实现过滤（.lower把大写字母转化为小写字母）
            # img_formats = ['.bmp', '.jpg', '.jpeg', '.png', '.tif', '.dng']
        ...
        # batch index
        # 将数据划分到一个个batch中，bi为batch列表，列表里放着一个个batch
        #假设n为6，则产生[0,1,2,3,4,5]除以3取整后[0,0,0,1,1,1]
        bi = np.floor(np.arange(n) / batch_size).astype(np.int)
        # 记录数据集划分后的总batch数，由上述例子，列表最后一个元素加1为batch数
        nb = bi[-1] + 1  # number of batches，[-1]为最后一个元素的位置
        ...
        sp = path.replace(".txt", ".shapes")  # shapefile path 把字符串的一部分替换成另一部分
        # tqdm库会显示处理的进度
        # 读取每张图片的size信息
        #经调试后确定执行了下面的if语句，没执行else
        #下面是进度条列表与图片路径列表之间的映射
        # if rank in [-1, 0]:
        #     image_files = tqdm(self.img_files, desc="Reading image shapes")
        # else:
        #     image_files = self.img_files
        #     print("else exe")
        # #s是一个元组列表，每个元组存储图片的宽和高
        # print("跳过tqdm")
        # s = [exif_size(Image.open(f)) for f in image_files]
        # print("按路径找图片成功，并成功创建图片的shape列表")
        # print(s[0])
        # # 将所有图片的shape信息保存在.shape文件中
        # np.savetxt(sp, s, fmt="%g")  # overwrite existing (if any)将array保存到txt文件 %g不输出无意义的零
        # print("处理异常，创建shape文件")
        try:
            with open(sp, "r") as f:  # read existing shapefile，s为现有的shape文件
                s = [x.split() for x in f.read().splitlines()]
                # 判断现有的shape文件中的行数(图像个数)是否与当前数据集中图像个数相等
                # 如果不相等则认为是不同的数据集，故重新生成shape文件
                #下面的语句打印出了0，说明存在我创建的空shape文件
                print(len(s))
